...
from configs.model_config import (VECTOR_SEARCH_SCORE_THRESHOLD,
                                  CHUNK_SIZE,
                                  )
...
class MyFAISS(FAISS, VectorStore):
    def __init__(
        ...
        self.chunk_content_expand = False
    ...
    def similarity_search_with_score_by_vector(
        ...
        for j, i in enumerate(indices[0]):
            ...
            # 根据chunk_id拿到 chunk content
            doc = self.docstore.search(_id)
            if (not self.chunk_content_expand) or ("context_expand" in doc.metadata and not doc.metadata["context_expand"]):
